...
import com.inventario.ui.ventas.FilaCarga;
...
public class SubirVentasPage extends BorderPane {
    ...
    private void procesarCargaMasiva() {
        ...
        for (FilaCarga fila : datosCarga) {
            ...
            if ("Registrado".equals(fila.getEstado())) continue;

            if (fila.getIdProducto() != null) {
                try {
                    double precio = ventaDAO.obtenerPrecioActual(fila.getIdProducto());

                    // CORRECCIÓN: Usar getters para cantidad, cliente, fecha
                    boolean ok = ventaDAO.registrarVenta(
                            fila.getIdProducto(),
                            fila.getCantidad(),
                            precio,
                            fila.getCliente(),
                            LocalDate.parse(fila.getFecha())
                    );
