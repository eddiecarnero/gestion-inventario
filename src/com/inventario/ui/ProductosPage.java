...
public class ProductosPage extends BorderPane {
    ...
    private void configurarTabla() {
        ...
        TableColumn<Producto, String> c1 = new TableColumn<>("Nombre"); c1.setCellValueFactory(new PropertyValueFactory<>("nombre"));
        TableColumn<Producto, String> c2 = new TableColumn<>("Proveedor"); c2.setCellValueFactory(new PropertyValueFactory<>("proveedorNombre"));
        ...
        c3.setCellFactory(tc -> new TableCell<>(){ @Override protected void updateItem(Double p, boolean e){super.updateItem(p,e); setText(e?null:String.format("$%.2f", p));}});

        TableColumn<Producto, Double> c4 = new TableColumn<>("Contenido");
        c4.setCellValueFactory(new PropertyValueFactory<>("contenido"));

        TableColumn<Producto, String> c5 = new TableColumn<>("Unidad"); c5.setCellValueFactory(new PropertyValueFactory<>("unidad"));
        TableColumn<Producto, Integer> c6 = new TableColumn<>("Stock Mín."); c6.setCellValueFactory(new PropertyValueFactory<>("stockMinimo"));

        tablaProductos.getColumns().addAll(c1, c2, c3, c4, c5, c6);
    ...
    private void cargarDatosTabla() {
        ...
        // IMPORTANTE: Leemos también la columna 'Contenido'
        ...
        try (Connection conn = ConexionBD.getConnection(); Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(sql)) {
            while (rs.next()) {
                Producto p = new Producto(
                        rs.getInt("IdProducto"), rs.getString("Tipo_de_Producto"), rs.getInt("Stock_Minimo"),
                        rs.getString("Unidad_de_medida"), rs.getString("Ubicacion"), rs.getInt("IdProveedor"),
        ...
        } catch (SQLException e) { e.printStackTrace(); mostrarAlerta("Error", "Error cargando productos."); }
    }
    ...
    private void guardarProducto() {
        ...
        try {
            ...
            // Leer contenido, por defecto 1 si está vacío o es 0
            double cont = 1.0;
            try { cont = Double.parseDouble(contenidoField.getText()); } catch(Exception ex){}
            if (cont <= 0) cont = 1.0;

            if (idProductoLabel.getText().isEmpty()) accionInsertar(nombre, idProv, precio, min, uni, ubi, cont);
            else accionActualizar(Integer.parseInt(idProductoLabel.getText()), nombre, idProv, precio, min, uni, ubi, cont);

        } catch (NumberFormatException e) { mostrarAlerta("Error", "Valores numéricos inválidos."); }
    }
    ...
    public static class Producto {
        private final int id, stockMinimo, idProveedor; private final String nombre, unidad, ubicacion; private final double precioUnitario, contenido; private String proveedorNombre;
        public Producto(int id, String n, int min, String u, String ub, int ip, double p, double c) {
            this.id=id; this.nombre=n; this.stockMinimo=min; this.unidad=u; this.ubicacion=ub; this.idProveedor=ip; this.precioUnitario=p; this.contenido=c; this.proveedorNombre="N/A";
        }
        public int getId(){return id;} public String getNombre(){return nombre;} public int getStockMinimo(){return stockMinimo;}
        public String getUnidad(){return unidad;} public String getUbicacion(){return ubicacion;} public int getIdProveedor(){return idProveedor;}
        public double getPrecioUnitario(){return precioUnitario;} public double getContenido(){return contenido;}
        public String getProveedorNombre(){return proveedorNombre;} public void setProveedorNombre(String n){proveedorNombre=n;}
    }
