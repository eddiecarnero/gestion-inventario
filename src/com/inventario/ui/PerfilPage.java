...
public class PerfilPage extends BorderPane {
    ...
    private Node crearTarjetaPerfil() {
        ...
        lblInicial.setTextFill(Color.WHITE);
        lblInicial.setFont(Font.font("Arial", FontWeight.BOLD, 20));
    ...
    private Node crearPanelGestionUsuarios() {
        ...
        txtTurno.setPromptText("Turno");
        txtHorario = new TextField();
        txtHorario.setPromptText("Horario");

        txtUser = new TextField();
        txtUser.setPromptText("Usuario Login");
        ...
        form.getChildren().addAll(lblForm, new Separator(), new Label("Personal:"), txtNombre, txtDNI, txtTel, new Label("Puesto:"), cmbRol, txtTurno, txtHorario, new Label("Acceso:"), txtUser, txtPass, new Separator(), btnGuardarUser, btnLimpiar, btnEliminarUser);
    ...
    private void configurarTablaUsuarios() {
        ...
        TableColumn<UsuarioModel, String> c1 = new TableColumn<>("Nombre");
        c1.setCellValueFactory(new PropertyValueFactory<>("nombre"));
        TableColumn<UsuarioModel, String> c2 = new TableColumn<>("Rol");
        c2.setCellValueFactory(new PropertyValueFactory<>("rol"));
        TableColumn<UsuarioModel, String> c3 = new TableColumn<>("Usuario");
        c3.setCellValueFactory(new PropertyValueFactory<>("user"));
    ...
    private void cargarUsuarios() {
        ...
        try (Connection c = ConexionBD.getConnection(); ResultSet rs = c.createStatement().executeQuery("SELECT * FROM empleado")) {
            while (rs.next()) {
                listaUsuarios.add(new UsuarioModel(rs.getInt("IdEmpleado"), rs.getString("Nombre_y_Apellido"), rs.getString("Tipo_de_empleado"), rs.getString("Telefono"), rs.getString("DNI"), rs.getString("Turnos"), rs.getString("Horario"), rs.getString("user"), rs.getString("password")));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void cargarUsuarioEnFormulario(UsuarioModel u) {
        ...
        txtTurno.setText(u.getTurno());
        txtHorario.setText(u.getHorario());
    ...
    private void guardarUsuario() {
        ...
        try (Connection c = ConexionBD.getConnection()) {
            if (idUsuarioEditando == null) {
                ...
                ps.setString(1, cmbRol.getValue());
    ...
    private void eliminarUsuario() {
        ...
        if ("Administrador".equals(cmbRol.getValue())) {
            mostrarAlerta("Error", "No puedes eliminar al Administrador principal.");
            return;
        }
    ...
    public static class UsuarioModel {
        int id;
        String nombre, rol, tel, dni, turno, horario, user, pass;
