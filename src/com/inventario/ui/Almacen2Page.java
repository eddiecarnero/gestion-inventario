...
public class Almacen2Page extends BorderPane {
    ...
    private void configurarTabla() {
        ...
        TableColumn<ProductoIntermedioAlmacen, String> col1 = new TableColumn<>("Producto");
        col1.setCellValueFactory(new PropertyValueFactory<>("nombre"));
        TableColumn<ProductoIntermedioAlmacen, Integer> col2 = new TableColumn<>("Cantidad");
        col2.setCellValueFactory(new PropertyValueFactory<>("stock"));
        col2.setCellFactory(c -> new TableCell<>() {
            @Override
            protected void updateItem(Integer i, boolean e) {
                super.updateItem(i, e);
                if (!e && i != null) {
                    setText(i.toString());
                    if (getTableView().getItems().get(getIndex()).esStockBajo()) getStyleClass().add("cell-stock-low");
                    else getStyleClass().remove("cell-stock-low");
                }
            }
        });
        TableColumn<ProductoIntermedioAlmacen, String> col3 = new TableColumn<>("Unidad");
        col3.setCellValueFactory(new PropertyValueFactory<>("unidad"));
        TableColumn<ProductoIntermedioAlmacen, Double> col4 = new TableColumn<>("Costo Unit.");
        col4.setCellValueFactory(new PropertyValueFactory<>("costoUnitario"));
        col4.setCellFactory(c -> new TableCell<>() {
            @Override
            protected void updateItem(Double p, boolean e) {
                super.updateItem(p, e);
                setText(!e && p != null ? String.format("$%.2f", p) : null);
            }
        });
        TableColumn<ProductoIntermedioAlmacen, Integer> col5 = new TableColumn<>("Stock Mín.");
        col5.setCellValueFactory(new PropertyValueFactory<>("stockMinimo"));
        TableColumn<ProductoIntermedioAlmacen, String> col6 = new TableColumn<>("Estado");
        col6.setCellValueFactory(new PropertyValueFactory<>("estado"));
        col6.setCellFactory(c -> new TableCell<>() {
            ...
            protected void updateItem(String s, boolean e) {
                ...
                if (!e && s != null) {
                    ...
                    b.getStyleClass().add(s.equals("Bajo Stock") ? "badge-stock-low" : "badge-stock-normal");
                    setGraphic(b);
    ...
    private void cargarDatos() {
        ...
        try (Connection conn = ConexionBD.getConnection(); Statement stmt = conn.createStatement()) {
            ...
            e.printStackTrace();
            mostrarAlerta("Error", "Error cargando datos.");
        }
    ...
    private void actualizarUIConDatos() {
        ...
        if (lowStockCount > 0) alertBoxContainer.getChildren().add(crearAlertBox(lowStockCount));
    }
    ...
    private void onUsarRecetaClicked() {
        ...
        dialog.setHeaderText("Producir un nuevo lote.");
        ...
        grid.add(new Label("Lotes:"), 0, 1);
        grid.add(txtCant, 1, 1);
    ...
    private void cargarRecetasParaDialog(ComboBox<RecetaSimple> combo) {
        ...
        try (Connection c = ConexionBD.getConnection(); ResultSet rs = c.createStatement().executeQuery(sql)) {
            while (rs.next())
                lista.add(new RecetaSimple(rs.getInt(1), rs.getString(2), rs.getDouble(3), rs.getString(4), rs.getInt(5)));
    ...
    private void procesarProduccion(RecetaSimple receta, int lotesAProducir, LocalDate fechaVencimiento) {
        ...
        try {
            ...
            String sqlIngredientes = "SELECT i.IdProducto, i.cantidad, i.unidad as UnidadReceta, p.Unidad_de_medida as UnidadStock " + "FROM ingredientes i " + "JOIN producto p ON i.IdProducto = p.IdProducto " + "WHERE i.receta_id = ?";
                ...
                try (PreparedStatement ps = conn.prepareStatement("SELECT IdProductoIntermedio FROM productos_intermedios WHERE Nombre = ?")) {
                    ...
                    if (rs.next()) idIntermedio = rs.getInt(1);
                        ...
                        try (PreparedStatement psIn = conn.prepareStatement("INSERT INTO productos_intermedios (Nombre, Unidad_de_medida) VALUES (?, ?)", Statement.RETURN_GENERATED_KEYS)) {
                            psIn.setString(1, receta.nombre());
                            psIn.setString(2, receta.unidad());
                            psIn.executeUpdate();
                            ResultSet rsk = psIn.getGeneratedKeys();
                            if (rsk.next()) idIntermedio = rsk.getInt(1);
                        }
                    }
                }
            }
            ...
            try (PreparedStatement ps = conn.prepareStatement("INSERT INTO lotes_intermedios (IdProductoIntermedio, CantidadActual, FechaVencimiento, FechaIngreso) VALUES (?, ?, ?, ?)")) {
                ...
                ps.setDouble(2, cantFinal);
                if (fechaVencimiento != null) ps.setString(3, fechaVencimiento.toString());
    ...
    public static class ProductoIntermedioAlmacen {
        private final int id, stock, stockMin;
