...
public class OrdenesPage extends BorderPane {
    ...
    private static final String CSS_STYLES = """
                ...
                .tab-content-area { -fx-padd120 0 0 0; }
    ...
    private Node crearTabNuevaOrden() {
        ...
        gridSup.add(crearCampo("Fecha Emisión", fechaPicker), 1, 0);
        ColumnConstraints col1 = new ColumnConstraints();
        col1.setPercentWidth(50);
        gridSup.getColumnConstraints().addAll(col1, col1);

        VBox addBox = new VBox(15);
        addBox.setStyle("-fx-border-color: #E0E0E0; -fx-border-radius: 8; -fx-padding: 15;");

        GridPane addGrid = new GridPane();
        addGrid.setHgap(15);
        insumoCombo.setPromptText("Producto");
        ...
        Button saveBtn = new Button("🛒 Guardar Orden (Pendiente)");
    ...
    private void configurarTablaItems(TableView<ItemOrden> table, ObservableList<ItemOrden> list, Label lblTotal) {
        ...
        c4.setCellFactory(tc -> new TableCell<>() {
            @Override
    ...
    private Node crearTabGestion() {
        ...
        TableColumn<ItemOrdenGestion, Integer> c1 = new TableColumn<>("ID");
        c1.setCellValueFactory(new PropertyValueFactory<>("id"));
        TableColumn<ItemOrdenGestion, String> c2 = new TableColumn<>("Proveedor");
        c2.setCellValueFactory(new PropertyValueFactory<>("proveedorNombre"));
        TableColumn<ItemOrdenGestion, String> c3 = new TableColumn<>("Fecha");
        c3.setCellValueFactory(new PropertyValueFactory<>("fechaTexto"));
        TableColumn<ItemOrdenGestion, Double> c4 = new TableColumn<>("Total");
        c4.setCellValueFactory(new PropertyValueFactory<>("total"));
        c4.setCellFactory(tc -> new TableCell<>() {
            @Override
            protected void updateItem(Double p, boolean e) {
        ...
        TableColumn<ItemOrdenGestion, String> c5 = new TableColumn<>("Estado");
        ...
        c6.setCellFactory(p -> new TableCell<>() {
            ...
            {
                b1.getStyleClass().add("button-accept");
                b1.setTooltip(new Tooltip("Recibir Mercadería"));
                b2.getStyleClass().add("button-edit");
                b2.setTooltip(new Tooltip("Editar Orden"));
                b3.getStyleClass().add("button-reject");
                b3.setTooltip(new Tooltip("Rechazar Orden"));
                b4.getStyleClass().add("button-print");
                b4.setTooltip(new Tooltip("Descargar/Imprimir PDF"));
    ...
    private void accionImprimirPDF(ItemOrdenGestion orden) {
        ...
        Label lblId = new Label(String.valueOf(orden.id()));
        lblId.setFont(Font.font("System", FontWeight.BOLD, 12));
        info.add(lblId, 1, 0);
        ...
        Label lblProv = new Label(orden.proveedorNombre());
        lblProv.setFont(Font.font("System", FontWeight.BOLD, 12));
    ...
    private void accionAceptarOrden(ItemOrdenGestion orden) {
        ...
        String sqlDet = "SELECT d.IdProducto, d.Cantidad, p.Tipo_de_Producto " + "FROM detalle_compra d " + "JOIN producto p ON d.IdProducto = p.IdProducto " + "WHERE d.IdCompra = ?";

        try (Connection conn = ConexionBD.getConnection(); PreparedStatement stmt = conn.prepareStatement(sqlDet)) {
            stmt.setInt(1, orden.id());
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                itemsParaRecibir.add(new ItemRecepcion(rs.getInt("IdProducto"), rs.getString("Tipo_de_Producto"), rs.getDouble("Cantidad")));
            }
        ...
        if (itemsParaRecibir.isEmpty()) {
            mostrarAlerta("Error", "La orden está vacía.");
            return;
        }
        ...
        tableRecepcion.setPrefHeight(300);
        tableRecepcion.setPrefWidth(500);
    ...
    private boolean procesarIngresoEnBD(int idOrden, List<ItemRecepcion> items) {
        ...
        try {
            ...
            try (PreparedStatement psLote = conn.prepareStatement(sqlLote); PreparedStatement psStock = conn.prepareStatement(sqlStock); PreparedStatement psKardex = conn.prepareStatement(sqlKardex)) {
            ...
            mostrarAlerta("Error Crítico", "Fallo al ingresar: " + e.getMessage());
            return false;
        } finally {
            if (conn != null) try {
    ...
    private void accionRechazarOrden(ItemOrdenGestion o) {
        try (Connection c = ConexionBD.getConnection(); PreparedStatement ps = c.prepareStatement("UPDATE orden_compra SET Estado='Rechazada' WHERE IdCompra=?")) {
            ps.setInt(1, o.id());
            ps.executeUpdate();
            mostrarAlerta("Info", "Orden rechazada.");
            cargarOrdenesPendientes();
        } catch (Exception e) {
    ...
    private void accionEditarOrden(ItemOrdenGestion orden) {
        ...
        // Cargar cabecera
        ComboBox<String> cProv = new ComboBox<>();
        cargarProveedores(cProv);
        cProv.setValue(orden.proveedorNombre());
        DatePicker cFecha = new DatePicker();
        try {
            cFecha.setValue(LocalDate.parse(orden.fechaTexto()));
        } catch (Exception e) {
        }

        // Cargar tabla temporal
        TableView<ItemOrden> tbl = new TableView<>();
        Label lblTot = new Label();
        ObservableList<ItemOrden> lst = FXCollections.observableArrayList();
        configurarTablaItems(tbl, lst, lblTot);
        cargarItemsDeOrden(orden.id(), lst);
        actualizarTotal(lst, lblTot);

        // Formulario para agregar MÁS items
        ComboBox<Insumo> cIns = new ComboBox<>();
        cIns.setCellFactory(lv -> new InsumoListCell());
        cIns.setButtonCell(new InsumoListCell());
        ...
        dialog.setResultConverter(btn -> {
            if (btn == ButtonType.OK) {
                if (lst.isEmpty()) {
                    mostrarAlerta("Error", "Mínimo 1 item");
                    return false;
                }
    ...
    private void cargarOrdenesPendientes() {
        ...
        try (Connection c = ConexionBD.getConnection(); ResultSet rs = c.createStatement().executeQuery("SELECT o.IdCompra, p.Nombre_comercial, o.Fecha_de_Compra, o.Precio_total, o.Estado FROM orden_compra o JOIN proveedores p ON o.IdProveedor = p.IdProveedor WHERE o.Estado = 'Pendiente' ORDER BY o.Fecha_de_Compra ASC")) {
            while (rs.next())
                listaOrdenesPendientes.add(new ItemOrdenGestion(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getDouble(4), rs.getString(5)));
        } catch (Exception e) {
        }
    }
    ...
    private void guardarOrden() {
        if (items.isEmpty()) {
            mostrarAlerta("Error", "Faltan items.");
            return;
        }
        try (Connection c = ConexionBD.getConnection()) {
            c.setAutoCommit(false);
            ...
            try (PreparedStatement ps = c.prepareStatement("INSERT INTO orden_compra (IdProveedor, IdEmpleado, Fecha_de_Compra, Precio_total, Estado) VALUES (?, ?, ?, ?, 'Pendiente')", Statement.RETURN_GENERATED_KEYS)) {
                ps.setInt(1, obtenerIdProveedor(proveedorCombo.getValue()));
                ps.setInt(2, 1);
                ps.setString(3, fechaPicker.getValue().toString());
                ps.setDouble(4, items.stream().mapToDouble(ItemOrden::getTotal).sum());
                ps.executeUpdate();
    ...
    private void cargarHistorial(VBox c) {
        ...
        try (Connection conn = ConexionBD.getConnection(); ResultSet rs = conn.createStatement().executeQuery("SELECT o.IdCompra, p.Nombre_comercial, o.Fecha_de_Compra, o.Precio_total FROM orden_compra o JOIN proveedores p ON o.IdProveedor=p.IdProveedor WHERE o.Estado!='Pendiente' ORDER BY o.Fecha_de_Compra DESC LIMIT 50")) {
            while (rs.next()) {
                ...
                c.getChildren().add(new VBox(l, new Separator()));
            }
        } catch (Exception e) {
        }
    }
    ...
    // --- UTILS ---
    private VBox crearCampo(String l, Node c) {
        VBox v = new VBox(5);
    ...
    private void agregarItemLogica(ComboBox<Insumo> c, TextField f, ObservableList<ItemOrden> l, Label t) {
        try {
            Insumo i = c.getValue();
            double q = Double.parseDouble(f.getText());
            if (i != null && q > 0) {
                boolean existe = false;
                for (ItemOrden io : l) {
                    if (io.getInsumoId() == i.getId()) {
                        io.setCantidad(io.getCantidad() + q);
                        existe = true;
                        break;
                    }
                }
    ...
    private void actualizarTotal(ObservableList<ItemOrden> l, Label t) {
        t.setText(String.format("$%.2f", l.stream().mapToDouble(ItemOrden::getTotal).sum()));
    }

    private void cargarProveedores(ComboBox<String> c) {
        try (Connection cn = ConexionBD.getConnection(); ResultSet rs = cn.createStatement().executeQuery("SELECT Nombre_comercial FROM proveedores")) {
            while (rs.next()) c.getItems().add(rs.getString(1));
        } catch (Exception e) {
        }
    }
    ...
    private int obtenerIdProveedor(String n) {
        try (Connection cn = ConexionBD.getConnection(); PreparedStatement p = cn.prepareStatement("SELECT IdProveedor FROM proveedores WHERE Nombre_comercial=?")) {
            p.setString(1, n);
    ...
    private void filtrarInsumosPorProveedor(ComboBox<Insumo> c, int id) {
        c.setItems(FXCollections.observableArrayList(todosLosInsumos.stream().filter(i -> i.getIdProveedor() == id).collect(Collectors.toList())));
    }

    // --- CLASES INTERNAS ---
    public static class Insumo {
        private final int id, idProv;
        private final String nombre, unidad;
        private final double precio, contenido;

        public Insumo(int i, int ip, String n, double p, String u, double cont) {
            id = i;
            idProv = ip;
            nombre = n;
            precio = p;
            unidad = u;
            contenido = (cont <= 0 ? 1 : cont);
        }
    ...
    private static class InsumoListCell extends ListCell<Insumo> {
        @Override
        protected void updateItem(Insumo i, boolean e) {
            super.updateItem(i, e);
            setText(i == null ? "" : i.toString());
        }
    }
    ...
    public static class ItemOrden {
        ...
        private final String nombre, unidad;
        private final double precioUnitario, total, contenido;
    ...
    public static class TestApp extends Application {
        ...
        public void start(Stage s) {
            s.setScene(new Scene(new OrdenesPage(), 1000, 800));
            s.show();
        }
    }
}